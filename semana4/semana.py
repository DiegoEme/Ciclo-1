def promedio_facultades(info: dict, contando_externos : bool = True) -> tuple:
    def validacion(s):
        sin_tildes = (("á", "a"), ("é", "e"), ("í", "i"), ("ó", "o"), ("ú", "u"),("ñ", "n"))
        for x, y in sin_tildes:s = s.replace(x, y).replace(x.upper(), y.upper())
        
        return s.lower()
    
    facultades, notas, creditos, correos, promedios = list(), list(), list(), set(), dict()
    
    try:
        for key, value in info.items():
            nombres, apellidos = validacion(value["nombres"]).split(), validacion(value["apellidos"]).split(", ")
            if not(not(contando_externos) and str(key)[4:6] == "05"):
                
                for materia in value["materias"]:
                    
                    if not(contando_externos == False and value["programa"] != materia["codigo"][:4]) and materia["retirada"] == "No"  and materia["creditos"] != 0:
                        if not(materia["facultad"] in facultades):
                            facultades.append(materia["facultad"])
                        try: 
                            tempo1, tempo2 = notas.pop(facultades.index(materia["facultad"])),creditos.pop(facultades.index(materia["facultad"]))
                        except: 
                            tempo1. tempo2 = 0, 0
                        notas.insert(facultades.index(materia["facultad"]), (tempo1+(materia["nota"]*materia["creditos"])))
                        creditos.insert(facultades.index(materia["facultad"]), (tempo2+materia["creditos"]))
                        
                        if len(nombres) == 1:
                            correos.add(nombres[0][:1]+apellidos[1][:1]+"."+apellidos[0]+str(value["documento"])[-2:])
                        else: 
                            correos.add(nombres[0][:1]+nombres[1][:1]+"."+apellidos[1]+str(value["documento"])[-2])
        
        for facultad in facultades:
            print(facultad)
            promedios[facultad] = round((notas[facultades.index(facultad)]/creditos[facultades.index(facultad)]),2)
        
        return (dict(sorted(promedios.items())), sorted(list(correos)))
